...
def retrieve_info(url: str) -> dict:
    ...
    for x in list(dev_rules.children):  # type: Tag
        ...
        if x.name == "h4":
            ...
            curr_name, curr_type = get_type_and_name(x, anchor, items, url, curr_section)
...
def get_type_and_name(t: Tag, anchor: Tag, items: dict, url: str, section: str):
    if t.text[0].isupper():
        curr_type = TYPES
    else:
        curr_type = METHODS
    curr_name = t.get_text()
    items[curr_type][curr_name] = {"name": curr_name, "section": section}
...
def generate_markdown(items: dict):
    os.makedirs("docs/methods", exist_ok=True)
    os.makedirs("docs/types", exist_ok=True)

    sections = {}

    # Generate Methods MD
    for name, data in items[METHODS].items():
        section = data.get("section", "Other")
        sections.setdefault(section, {METHODS: [], TYPES: []})
        sections[section][METHODS].append(name)

        content = f"# Method: {name}\n\n"
        if "section" in data:
            content += f"**Category**: {data['section']}\n\n"
        if "description" in data:
            desc = "\n".join(data["description"])
            content += f"**Description**: {desc}\n\n"
        if "href" in data:
            content += f"**Link**: [Official Documentation]({data['href']})\n\n"
        if "returns" in data:
            content += f"**Returns**: {', '.join(data['returns'])}\n\n"
        ...
        if "fields" in data and data["fields"]:
            ...
            for field in data["fields"]:
                ftypes = " or ".join(field["types"])
                req = "Yes" if field["required"] else "Optional"
                fdesc = field["description"].replace("\n", " ")
                content += f"| {field['name']} | {ftypes} | {req} | {fdesc} |\n"
                
        with open(f"docs/methods/{name}.md", "w") as f:
            f.write(content)

    # Generate Types MD
    for name, data in items[TYPES].items():
        section = data.get("section", "Other")
        sections.setdefault(section, {METHODS: [], TYPES: []})
        sections[section][TYPES].append(name)
        ...
        if "subtypes" in data and data["subtypes"]:
            content += f"**Subtypes**: {', '.join(data['subtypes'])}\n\n"
            
        if "subtype_of" in data and data["subtype_of"]:
            content += f"**Subtype of**: {', '.join(data['subtype_of'])}\n\n"
        ...
        with open(f"docs/types/{name}.md", "w") as f:
            f.write(content)

    # Generate Index MD
    index_content = "# Telegram Bot API Index\n\n"
    index_content += "This index categorizes all methods and types found in the Telegram Bot API documentation.\n\n"
    
    for section, parts in sections.items():
        index_content += f"## {section}\n\n"
        
        if parts[METHODS]:
            index_content += "### Methods\n"
            for m in sorted(parts[METHODS]):
                index_content += f"- [{m}](methods/{m}.md)\n"
            index_content += "\n"
            
        if parts[TYPES]:
            index_content += "### Types\n"
            for t in sorted(parts[TYPES]):
                index_content += f"- [{t}](types/{t}.md)\n"
            index_content += "\n"
