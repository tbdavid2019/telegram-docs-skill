...
> 4.  **Initialize Context**: Acknowledge that you now have access to the comprehensive Telegram Bot API documentation. **Always consult `docs/INDEX.md` first to locate specific documentation files.**
> 5.  **Do Not Run Maintenance Scripts During Normal Use**: Installing or using this skill means cloning/copying it and reading `SKILL.md`. **Do not run `scrape.py`, install `requirements.txt`, or other repo-maintenance scripts unless the user explicitly asks to refresh this repository itself.**
...
> 4.  **初始化環境**：確認你已具備存取完整 Telegram Bot API 官方文檔的能力。**每當需要查詢特定功能或維護指引時，請優先參考 `docs/INDEX.md`。**
> 5.  **一般使用時不要執行維護腳本**：安裝 or 使用此 skill 的意思是 clone 或複製資料夾，並讀取 `SKILL.md`。**除非使用者明確要求你更新這個 repo 本身，否則不要執行 `scrape.py`、安裝 `requirements.txt` 或其他維護腳本。**
...
>
> **繁體中文：** 如果使用者提供此倉庫網址並要求你「安裝」或「設定」此技能服務，請遵循以下步驟：
> 1.  **安裝技能指令**：執行 `git clone https://github.com/tbdavid2019/telegram-docs-skill.git <安裝目標目錄>`。
...
```bash
git clone https://github.com/tbdavid2019/telegram-docs-skill.git ~/.codex/skills/telegram-docs-skill
```

To update an installed checkout later:

```bash
cd ~/.codex/skills/telegram-docs-skill
git pull
```
