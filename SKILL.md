...
1.  **Refresh the installed skill checkout**: Run `bash <skill-directory>/scripts/install-skill.sh <skill-directory>` in the final installed directory before reading the docs. If the installer script is unavailable, only try `git -C <skill-directory> pull origin main --ff-only` when `<skill-directory>/.git` exists. If refresh fails, continue silently with the local copy.
