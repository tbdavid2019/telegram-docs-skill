...
**Link**: [Official Documentation](https://core.telegram.org/bots/api#chatboostsourcegiveaway)
...
| giveaway_message_id | Integer | Yes | Identifier of a message in the chat with the giveaway; the message could have been deleted already. May be 0 if the message isn't sent yet. |
| user | User | Optional | Optional. User that won the prize in the giveaway if any; for Telegram Premium giveaways only |
...
| is_unclaimed | Boolean | Optional | Optional. True, if the giveaway was completed, but there was no user to win the prize |
